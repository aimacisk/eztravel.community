"""
DOM 探查 — trip.eztravel.com.tw 景點頁
輸出：scripts/dom_spot.json
"""
import json
...
with sync_playwright() as pw:
    ...
    () => {
        const els = Array.from(document.querySelectorAll('body *')).slice(0, 500);
        const keywords = ['search', 'banner', 'hero', 'spot', 'trip', 'dest',
                          'theme', 'card', 'product', 'feature', 'recommend',
                          'section', 'slider', 'swiper', 'wrapper', 'layout',
                          'article', 'widget', 'hot', 'promo', 'quick', 'link'];
        const found = [];
        for (const el of els) {
            const cls = el.className && typeof el.className === 'string'
                ? el.className.trim() : '';
            const id = el.id || '';
            const skipTags = ['script','style','svg','path','use'];
            if ((cls || id) && !skipTags.includes(el.tagName.toLowerCase())) {
    ...
    out = Path(__file__).parent / "dom_spot.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump({"structure": structure, "class_list": class_list}, f, ensure_ascii=False, indent=2)
    ...
    for c in structure.get('children', []):
        ...
        for cc in c.get('children', []):
            ...
            for ccc in cc.get('children', []):
                cls3 = (ccc['cls'] or '')[:50]
                print(f"      [{ccc['tag']}] id={ccc['id']} cls={cls3!r} imgs={ccc['imgCount']}")

    print("\n--- Key class names ---")
    for item in class_list[:60]:
        print(f"  [{item['tag']}] id={item['id']} cls={item['cls'][:80]!r} imgs={item['imgCount']} | {item['text'][:40]!r}")
